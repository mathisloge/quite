...
# compatibility/availability with the current compiler.
#
...
function(set_sanitizer_options SANITIZER_NAME)
    ...
    if(
        ...
        set(SANITIZER_${UPPER_SANITIZER_NAME}_OPTIONS
            ...
            CACHE STRING
            "${LOWER_SANITIZER_NAME} sanitizer compile options"
            FORCE
        )
        ...
        if(SANITIZER_${UPPER_SANITIZER_NAME}_AVAILABLE)
            ...
            elseif(
                NOT SANITIZERS_AVAILABLE_LIST MATCHES "(${LOWER_SANITIZER_NAME})"
            )
                set(SANITIZERS_AVAILABLE_LIST
...
# compatibility and adds the compile_options and link_options
# ~~~
# Required:
...
function(add_sanitizer_support)
    ...
    foreach(SELECTED_SANITIZER ${ARGN})
        ...
        if(SANITIZER_${UPPER_SANITIZER_NAME}_AVAILABLE)
            ...
            if(AFL)
                if(
                    ${SANITIZER_${UPPER_SANITIZER_NAME}_SANITIZER}
                        MATCHES
                        "address"
                )
                ...
                elseif(
                    ${SANITIZER_${UPPER_SANITIZER_NAME}_SANITIZER} MATCHES "cfi"
                )
            ...
            message(
                SEND_ERROR
                "'${LOWER_SANITIZER_NAME}' sanitizer set not available"
            )
        endif()
    endforeach()
    ...
    if(
        NOT DEFINED SANITIZER_COMPILE_OPTIONS_GLOBAL_CACHE
        OR NOT SANITIZER_COMPILE_OPTIONS_SELECTED
            STREQUAL
            SANITIZER_COMPILE_OPTIONS_GLOBAL_CACHE
    )
        # set of flags needs to be tested for compatibility
    ...
    if(SANITIZER_SELECTED_OPTIONS_AVAILABLE)
        ...
        message(
            FATAL_ERROR
            "Selected sanitizer options not compatible: ${ARGN}"
        )
    endif()
endfunction()
